...
- (void)setupUI {
    ...
    upBtn.tag = TuyaSmartPTZControlDirectionUp;
    
    UIButton *downBtn = [self creatButtonWithTitle:NSLocalizedStringFromTable(@"DOWN", @"IPCLocalizable", @"") tag:TuyaSmartCameraPTZDirectionDown];
    downBtn.frame = CGRectMake(centerX-0.5*btnWidth, centerY+verticalMargin, btnWidth, btnHeight);
    downBtn.tag = TuyaSmartPTZControlDirectionDown;
    
    UIButton *leftBtn = [self creatButtonWithTitle:NSLocalizedStringFromTable(@"LEFT", @"IPCLocalizable", @"") tag:TuyaSmartCameraPTZDirectionLeft];
    leftBtn.frame = CGRectMake(centerX-btnWidth-horizonMargin, centerY-btnHeight*0.5, btnWidth, btnHeight);
    leftBtn.tag = TuyaSmartPTZControlDirectionLeft;
    
    UIButton *rightBtn = [self creatButtonWithTitle:NSLocalizedStringFromTable(@"RIGHT", @"IPCLocalizable", @"") tag:TuyaSmartCameraPTZDirectionRight];
    rightBtn.frame = CGRectMake(centerX+horizonMargin, centerY-btnHeight*0.5, btnWidth, btnHeight);
    rightBtn.tag = TuyaSmartPTZControlDirectionRight;
...
//    presetBtn1.frame = CGRectMake(addCPBtn.frame.origin.x, addCPBtn.frame.origin.y + addCPBtn.frame.size.height + 10, 160, btnHeight);
...
//    presetBtn2.frame = CGRectMake(presetBtn1.frame.origin.x, presetBtn1.frame.origin.y + presetBtn1.frame.size.height + 10, 160, btnHeight);
...
//        __weak typeof(self) weakSelf = self;
        [self.ptzManager setPresetPointWithIndex:index success:^(id result) {
            NSLog(@"success");
            [self showAlertWithMessage:NSLocalizedStringFromTable(@"Success", @"IPCLocalizable", @"") complete:nil];
        } failure:^(NSError *error) {
            NSLog(@"error");
        }];
    }
